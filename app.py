    ...
    [
        ...
        html.Div(
            [
                ...
                # NOTE: title
                html.Div([html.H1("PBT Admin Dashboard")], className="title-container"),
                # NOTE: external link
                html.Div(
                    [
                        html.A(
                ...
                html.Div(
                    [
                        dcc.Dropdown(
                            id="month-filter",
                            options=[
                                {"label": key, "value": month_dict[key]}
        ...
        html.Div(
            [
                ...
                html.Div(
                    [
                        html.H2(id="total-new-users-sessions"),
                        html.H4("Sessions By New Users"),
                    ],
                    id="new-users",
                    className="stat-container",
                ),
                    ...
                    [
                        dcc.Dropdown(
                            ...
                            options=[
                                ...
                                for i in ["student", "teacher", "alumnus"]
                            ],
                ...
                html.Div(
                    [
                        dcc.Dropdown(
                            ...
                            options=[
                                {"label": i.capitalize(), "value": i}
                                for i in ["Bar Chart", "Line Chart", "Scatter Chart"]
                            ],
                            value="Bar Chart",
                            clearable=False,
                        )
        ...
        html.Div(
            [
                # NOTE: course filter
                html.Div(
                    id="data-chart-course-filter-container",
                    children=[
                        dcc.Dropdown(
...
def get_and_store_login_data(selected_month):
    ...
    hidden_data_div = html.Div(
        id="jsonified-login-df", children=jsonified_data, style={"display": "none"}
    )
...
def get_and_store_usage_data(selected_month):
    ...
    jsonified_data = df.to_json(date_format="iso")
    hidden_data_div = html.Div(
        id="jsonified-data-usage-df", children=jsonified_data, style={"display": "none"}
    )
...
def update_login_chart_on_filter_change(
    jsonified_df, association, status, chart_type, frequency, month
):
    if jsonified_df is None:
        ...
        if association == None:
            if status == None:
                fig = chart_helper.make_login_chart(
                    df, month, frequency=frequency[0], chart_type=chart_type
                )
                # print(fig.data)
                return fig
            else:
...
@app.callback(
    ...
    [Input("jsonified-login-df", "children"), Input("association-filter", "value")],
)
...
def display_total_distinct_users_by_association(jsonified_df, association):
    if jsonified_df is None:
        ...
        return chart_helper.get_total_distinct_users_by_association(
            jsonified_df, association
        )
...
def display_total_new_users_sessions_by_association(jsonified_df, association):
    if jsonified_df is None:
        ...
        return chart_helper.get_total_new_users_sessions_by_association(
            jsonified_df, association
        )
...
def display_total_distinct_users_by_association(jsonified_df, association):
    if jsonified_df is None:
        ...
        return chart_helper.get_total_data_usage_by_association(
            jsonified_df, association
        )
...
def display_total_new_resources_by_association(jsonified_df, association):
    if jsonified_df is None:
        ...
        return chart_helper.get_total_resources_by_association(
            jsonified_df, association
        )
